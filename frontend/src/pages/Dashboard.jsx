...
import { Cpu, HelpCircle, FileText, Settings, Award, AlertCircle, TrendingUp, Calendar, ChevronRight } from 'lucide-react';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        // Get user profile statistics (aggregations calculated on-demand)
        ...
        if (historyRes.data.success) {
          setRecentInterviews(historyRes.data.data.slice(0, 3)); // show top 3 recent
        }
      } catch (err) {
        console.error('Error fetching dashboard data:', err);
        setToast({ message: 'Failed to retrieve dashboard analytics. Database offline.', type: 'warning' });
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-brand-border dark:border-dark-border pb-6">
        <div>
          <h1 className="font-outfit font-extrabold text-3xl text-brand-charcoal dark:text-dark-text flex items-center gap-2.5">
            <TrendingUp className="text-brand-purple dark:text-dark-purple" size={30} />
            <span>Welcome, <span className="text-brand-purple dark:text-dark-purple">{user?.name}</span>!</span>
          </h1>
          <p className="text-brand-slate dark:text-dark-muted text-sm">Monitor your placement metrics and take simulated interviews.</p>
        </div>
        <Link
          to="/interview/setup"
